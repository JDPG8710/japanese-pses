...
try {
  ...
  async function setup(p,name){await p.goto(`${preview.origin}/arena.html?lang=zh`);await p.locator('[data-consent=necessary]').click();await p.locator('#playroom-profile [name=nickname]').fill(name);await p.locator('#playroom-profile [name=visible]').check();await p.locator('#playroom-profile button').click();await p.locator('#playroom-profile').waitFor({state:'hidden'});}
  ...
  await a.locator('[data-action=resign]').click();await a.locator('[data-action=home]').click();await a.locator('.family-panel').waitFor();checks++;
  await a.locator('[data-pr=learn]').click();await a.locator('[data-learn-point="0"]').click();await a.locator('[data-learn=next]').click();
  ...
  await b.setViewportSize({width:390,height:844});await b.locator('#locale').selectOption('ja');await b.locator('[data-pr=learn]').click();
