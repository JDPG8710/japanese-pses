...
import {GAMES,makeRounds,checkAnswer,solution,evaluate,isSet,robotState} from '../src/world/WorldRules.mjs';

let checks=0;const check=(value,message)=>{assert.ok(value,message);checks++;};
const oldGames=['cafe','garden','rhythm','data','matrix','memory','switch'];
...
function wrongAnswer(game,q,good){
 ...
 if(game==='robot')return good.slice(0,-1);
...
for(const game of GAMES)for(const level of [1,2])for(let seed=0;seed<100;seed++){
 ...
 for(const q of rounds){
  ...
  if(game==='robot'){
   const state=robotState(q,good),direct=Math.abs(q.goal[0]-q.start[0])+Math.abs(q.goal[1]-q.start[1]);
   check(q.blocks.length>0,'robot field contains blocking obstacles');
   check(good.length===q.limit&&good.length>direct,'robot requires an obstacle-forced optimal route');
   check(state.valid&&state.position[0]===q.goal[0]&&state.position[1]===q.goal[1],'robot optimal route reaches the goal safely');
  }
...
const robotQ={size:3,start:[0,1],goal:[2,1],blocks:[[1,1]],limit:4};
check(checkAnswer('robot',robotQ,[0,1,1,2]),'robot accepts a shortest route around an obstacle');
check(!robotState(robotQ,[1]).valid&&!checkAnswer('robot',robotQ,[1,1]),'robot obstacle blocks movement');
check(!checkAnswer('robot',robotQ,[0,2,0,1,1,2]),'robot rejects a safe route that is longer than optimal');
...
try{
 const db=await mf.getD1Database('DB');for(const file of ['migrations/0001_d1_data_platform.sql','migrations/0005_world_games.sql','migrations/0006_foundation_games.sql','migrations/0007_world_games_brain_arcade.sql'])await statements(db,file);
 ...
 await statements(db,'migrations/0009_tutorial_pause.sql');
 ...
 run=await api('start',{game:'robot',level:1},'b');check((await api('answer',{id:run.id,revision:0,answer:Array(5000).fill(1)},'b')).status===413,'request body remains bounded');await db.prepare('UPDATE auth_sessions SET revoked_at=1 WHERE jti=?1').bind('b').run();check((await api('answer',{id:run.id,revision:0,answer:[1]},'b')).status===401,'revoked login rejected');
 console.log(`Logic Lab: ${checks} strict checks passed (1,800 generated sessions / 18,000 solution-withheld puzzles, real D1 migration and ranked API).`);
