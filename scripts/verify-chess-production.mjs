...
try{
  ...
  async function setup(page,name){await page.goto('https://piko-game.com/arena.html?lang=zh',{waitUntil:'networkidle'});await page.locator('[data-consent=necessary]').click();check(await page.locator('#title').innerText()==='Piko Playroom','production brand');await page.locator('#playroom-profile [name=nickname]').fill(name);check(!await page.locator('#playroom-profile [name=visible]').isChecked(),'QA guest remains private');await page.locator('#playroom-profile button').click();await page.locator('#playroom-profile').waitFor({state:'hidden'});await page.goto('https://piko-game.com/arena.html?game=chess&lang=zh',{waitUntil:'networkidle'});page.on('dialog',dialog=>dialog.accept());}
