...
- ゲスト体験は端末側 IndexedDB と Worker 側 D1 の両方で、ステージ実行中かつページ表示中だけ週2時間へ累積し、7日周期で更新されることを確認する。メニュー、結果画面、バックグラウンドの経過時間を消費させず、元のIPや端末信号を保存させない。
...
- D1 の教材、カリキュラム、利用者、OAuth、セッション、ゲスト、進捗、挑戦履歴、卒業証テーブルと IndexedDB オフラインキャッシュが契約どおりである。
