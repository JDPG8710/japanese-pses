...
export async function foundationRoute(request,env,{authenticate,json,HttpError}){
 ...
 if(request.method!=='POST'||!['/api/foundation/start','/api/foundation/answer','/api/foundation/tutorial'].includes(url.pathname))throw new HttpError(404,'NOT_FOUND');
 ...
 const now=Date.now();
 if(url.pathname==='/api/foundation/tutorial')return reply(await tutorialPause(db,'foundation_runs',body,session.sub,now,HttpError));
 ...
 const changed=await db.prepare('UPDATE foundation_runs SET round_index=?1,tries=?2,score=?3,revision=revision+1,completed_at=?4 WHERE run_id=?5 AND user_id=?6 AND revision=?7 AND completed_at IS NULL AND tutorial_paused_at IS NULL AND expires_at>?8').bind(index,verdict.tries,score,index===10?now:null,run.run_id,session.sub,body.revision,now).run();
