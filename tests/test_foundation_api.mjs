...
try{
 const db=await mf.getD1Database('DB');for(const file of ['0001_d1_data_platform.sql','0005_world_games.sql','0006_foundation_games.sql','0006_foundation_games.sql','0009_tutorial_pause.sql'])for(const sql of (await readFile(`migrations/${file}`,'utf8')).split(/;(?=(?:[^']*'[^']*')*[^']*$)/).map(s=>s.trim()).filter(Boolean))await db.prepare(sql).run();
