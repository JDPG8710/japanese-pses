...
export class TownScene {
 constructor(canvas,options){
  Object.assign(this,options);this.canvas=canvas;this.keys=new Set();this.path=[];this.position=new THREE.Vector3(toWorld(this.state.player).x,0,toWorld(this.state.player).z);this.velocity=0;this.grounded=true;this.near=null;this.clock=0;this.yaw=.22;this.pitch=.58;this.distance=22;this.mode=null;this.cooldown=0;this.characterKey='';this.viewMode=this.state.expansion?.cameraMode||'third';this.firstPitch=.08;this.jumpCount=0;this.platformId=null;
 ...
 resetCamera(){this.yaw=this.mode?0:.22;this.pitch=.5;this.firstPitch=.08;this.distance=this.mode?14:22;}
 ...
 enterGame(mode,q,stage,onChoice,onFall,run){this.stop();this.mode=mode;this.question=q;this.onChoice=onChoice;this.onFall=onFall;this.clearEnvironment();this.resetCamera();this.distance=14;this.yaw=0;
  ...
  if(mode==='obby'||mode==='tower'){
   ...
   for(const [i,p]of this.course.platforms.entries()){
    ...
    if(p.index!==undefined){this.targets.push({...p});label(this.environment,q.options[p.index],p.x,p.y+1.9,p.z,{width:3.4,size:86});box(this.environment,p.x,p.y+.025,p.z,p.w*.72,.05,p.d*.72,0xffedb5);}
 ...
 render(){
  ...
  this.sun.position.set(this.position.x-12,this.position.y+25,this.position.z+14);this.sun.target.position.copy(this.position);this.sun.target.updateMatrixWorld();this.renderer.render(this.world,this.camera);
 }
}
