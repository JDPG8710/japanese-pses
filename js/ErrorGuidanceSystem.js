...
export class ErrorGuidanceSystem {
  ...
  generateSubjectClue(context = {}) {
    ...
    switch (subject) {
      ...
      case '外国語・英語':
      case '英語':
        return 'イラストの場面と、えいごのはじめの音を見て、意味をつなげてみよう！';
    ...
    switch (subject) {
      ...
      case '英語':
        if (qData.correctAnswer) {
          return `ちょっと迷ったね。まずは しゅるい をよく見て、きもちを つなぐ と すぐわかるよ。 もう一回、しっかり みてみよう！`;
        }
        return 'イラストの場面をよく見て、えいごの意味を つなげて考えてみよう。ゆっくり、もう一回だよ！';

      default:
        return 'ちょっと難しかったね。ゆっくり もう一回 みてみよう。だいじょうぶ、できるよ！';
    }
  }
}
