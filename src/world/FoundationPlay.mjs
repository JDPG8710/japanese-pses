...
function rich(value){return esc(value);}
...
async function start(){void recordPlay(`lesson:${route.lesson}`);stop();const token=epoch;busy=true;view='intro';feedback=w.loading;render();try{
 ...
 if(!await tutorial.open(run,foundationTutorial(TOPICS[route.lesson].title[locale],locale,run.question.kind,w.rules))||token!==epoch)return;
 view='play';busy=false;feedback='';verdict=null;render();timer=setInterval(tick,250);
...
async function showTutorial(){
 if(!run||busy||tutorial.paused||verdict?.done&&run.pending?.complete)return;
 tick();if(view!=='play')return;const token=epoch;busy=true;
 try{await tutorial.open(run,foundationTutorial(TOPICS[route.lesson].title[locale],locale,run.question.kind,w.rules));}
 catch{if(token===epoch){stop();view='intro';feedback=w.error;render();}}
 finally{if(token===epoch){busy=false;tick();}}
}
