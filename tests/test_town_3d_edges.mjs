...
try{
 ...
 await page.locator('[data-arcade-shop]').click();for(const id of ['shoes','spring','shield','hint'])await page.locator(`[data-buy-item="${id}"]`).click();assert.equal((await read(page)).coins,94);assert.equal(await page.locator('[data-buy-item="shoes"]').isDisabled(),true);await page.locator('.close-button').click();mark('gear purchases deduct exact prices and cannot be purchased twice');
 await page.locator('[data-game="obby"]').click();let run=(await read(page)).expansion.runs.obby,q=questionFor(run);await page.locator(`[data-answer-platform="${(q.answer+1)%3}"]`).click();await page.waitForFunction(()=>JSON.parse(localStorage.getItem('piko-town-v1')).expansion.inventory.shield===0,{},{timeout:25000});assert.equal((await read(page)).expansion.runs.obby.hearts,3);assert.equal((await read(page)).coins,94);await page.locator('[data-arcade-hint]').click();assert.equal((await read(page)).expansion.inventory.hint,0);assert.ok(await page.locator('.arcade-panel .hint-box').isVisible());mark('wrong physical answer consumes exactly one shield; hint card reveals explanation');
 ...
 for(const mode of ['obby','tower']){
  if(mode==='tower'){await page.locator('[data-exit-game]').last().click();await page.locator('[data-game="tower"]').click();}
  for(const target of [0,1,2]){
   ...
   const previous=r.hearts;await page.locator(`[data-answer-platform="${target}"]`).click();await page.waitForFunction(({mode,previous})=>{const r=JSON.parse(localStorage.getItem('piko-town-v1')).expansion.runs[mode];return r.solved||r.hearts<previous;},{mode,previous},{timeout:25000});
 ...
 }mark('all six left / middle / right landing routes work with speed shoes and spring boots');
 await page.locator('[data-exit-game]').last().click();await page.reload();await page.locator('[data-game="memory"]').click();await page.locator('[data-answer-platform="0"]:not([disabled])').waitFor({timeout:12000});const before=await read(page);await page.locator('[data-memory-replay]').click();assert.equal((await read(page)).coins,before.coins);assert.equal((await read(page)).expansion.runs.memory.memoryIndex,0);mark('memory replay is free, resets sequence position and grants no reward');
